     ...
     <md-icon style="color:grey">info</md-icon>
      <md-tooltip  md-direction="top"> <md-icon style="color:white;">keyboard</md-icon> รหัสสินค้า หรือ ชื่อสินค้า + Enter</md-tooltip>
                    ...
                    <md-table-row @click="checkval(item)" slot="md-table-row"  slot-scope="{ item }">
                      ...
                      <md-table-cell md-label="ราคา/หน่วย" md-sort-by="price"><input type="text" disabled class="datatable" @keyup="calculatedata(item)" style="width:100%" v-model="item.price"></md-table-cell>
