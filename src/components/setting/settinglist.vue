    ...
    <div class="container">
      <div class="col-12">
        <md-field>
          ...
          <label>ค้นหา</label>
          <md-input @keyup.enter="searchSet" v-model="searchBykey"></md-input>
        </md-field>
      </div>
      ...
      <md-speed-dial class="md-bottom-right">
        <md-speed-dial-target @click="createSet">
          <md-icon>add</md-icon>
...
export default {
  ...
  methods: {
    ...
    createSet() {
      this.$router.push({ name: "setting", params: { id: 0 } });
    },
    editSet(val) {
      console.log(val.id)
      this.$router.push({ name: "setting", params: { id: val.id } });
    },
    searchSet() {
      var payload = {
        keyword: this.searchBykey
      };

      console.log(JSON.stringify(payload));
      api.searchSetByKeyword(
        payload,
        result => {
